import QuestionGBP # this imports the Question file to allow it to be accessed but this section of code
import UnrecognisedGBP # this imports the Unrecognised file to allow for unrecognised input to be detected
...
def selection():
   ...
   Sorry, Snickers is out of stock""")
                  QuestionGBP.question() # this directs the code to the question file to ensure that no further changes are made to the current saved totals
             list.append('Snickers') # adds "Snickers" to the current list displaying purchases 
             total = total+0.99 # adds 0.99 (signifying 99p) to the current total and saves it as the new current total 
             print ("""
   For""",list,"your current total is £", ("%.2f" % total)) # this prints an current recipt for the user showing the updated list with thier purchaes and the current total price of all items to two decimal places to show currency
             QuestionGBP.question() # this will direct the code to the question file to ask if the user wants to add anything else to thier current order
...
def pay(): 
   ...
   Input 0.00 to quit""") # repeats the current list and total to repeat the question as "while True" is still active 
                  ...
                  UnrecognisedGBP.main() # re-directs to the unrecognised sections 
   ...
   press the enter key to exit""")
      ...
      QuestionGBP.question() #re-directs to the question section 
   ...
   Press the enter key to exit""") # allows the user to quit using the enter key
      ...
      UnrecognisedGBP.main() # re-directs to the unrecognised input section. 
